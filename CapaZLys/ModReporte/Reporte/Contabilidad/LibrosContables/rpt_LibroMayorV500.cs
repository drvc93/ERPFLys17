...
namespace FiltroLys.ZLys.ModReporte.Reporte.Contabilidad.LibrosContables
{
    public partial class rpt_LibroMayorV500 : DevExpress.XtraReports.UI.XtraReport
    {
        ...
        {
            String sVouch = "";
            sVouch = GetCurrentColumnValue("c_voucher").ToString();
            if (sVouch.Length > 0){
                nDebeGR += Convert.ToDecimal(GetCurrentColumnValue("n_debe"));
            }
        }
        ...
        private void xrGRVoucherHaber_SummaryRowChanged(object sender, EventArgs e)
        {
            ...
            sVouch = GetCurrentColumnValue("c_voucher").ToString();
            if (sVouch.Length > 0){
                nHaberGR += Convert.ToDecimal(GetCurrentColumnValue("xrHaber"));
            }
        }
        ...
        {
            nDebeSal += Convert.ToDecimal(GetCurrentColumnValue("n_debe"));
            nHaberSal += Convert.ToDecimal(GetCurrentColumnValue("xrHaber"));
        }
        ...
        private void xrGRMovimientoMes_SummaryRowChanged(object sender, EventArgs e)
        {
            ...
            sVouch = GetCurrentColumnValue("c_voucher").ToString();
            if (sVouch.Length > 0){
                nDebeMv += Convert.ToDecimal(GetCurrentColumnValue("n_debe"));
                nHaberMv += Convert.ToDecimal(GetCurrentColumnValue("xrHaber"));
            }
        }
        ...
        {
            nDebeGR2 += Convert.ToDecimal(GetCurrentColumnValue("n_debe"));
            nHaberGR2 += Convert.ToDecimal(GetCurrentColumnValue("xrHaber"));
        }
